        ...
        >
            {({ isDisabled, isFocusVisible, isSelectionStart, isSelectionEnd, isSelected, formattedDate }) => {
                ...
                return (
                    <div
                        className={cx(
                            /* tabular-nums + stable weight: avoid grid “bump” when selection/hover toggles font-medium */
                            "relative flex size-full items-center justify-center rounded-full text-sm font-medium tabular-nums",
                            // Disabled state.
                            isDisabled ? "text-disabled" : "text-secondary hover:text-secondary_hover",
                            // Focus ring, visible while the cell has keyboard focus.
                            isFocusVisible ? "outline-2 outline-offset-2 outline-focus-ring" : "",
                            markedAsSelected && "bg-brand-solid text-white hover:bg-brand-solid_hover hover:text-white",
                            // Hover state for non-selected cells (background only — no weight change).
                            !isSelected && !isDisabled ? "hover:bg-primary_hover" : "",
                            !isSelected && isTodayDate ? "bg-active hover:bg-secondary_hover" : "",
                        )}
                    >
                        {formattedDate}
